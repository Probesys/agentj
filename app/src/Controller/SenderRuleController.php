...
class SenderRuleController extends AbstractController
{
    ...
    #[Route(path: '/sender-rule/{type}', name: 'sender-rule')]
    public function index(string $type, Request $request, PaginatorInterface $paginator): Response
    {
        ...
        $filterForm = $this->createForm(ActionsFilterType::class, null, [
            ...
            'action' => $this->generateUrl('sender_rule_batch'),
        ]);
    ...
    #[Route(path: '/sender-rule/{rid}/{sid}/{priority}/delete', name: 'sender_rule_delete', methods: 'GET')]
    ...
    #[Route(
        path: '/sender-rule/batch/delete/',
        name: 'sender_rule_batch_delete',
        methods: 'POST',
        options: ['expose' => true],
    )]
    public function batchDeleteAction(Request $request): RedirectResponse
    {
        ...
        foreach ($request->request->all('id') as $obj) {
            ...
            $this->em->getRepository(SenderRule::class)->delete($mailInfo[0], $mailInfo[1], $mailInfo[2]);
        }
    ...
    #[Route(path: '/sender-rule/admin/import/{type}', name: 'import_sender_rule', options: ['expose' => true])]
    public function importSenderRuleAction(Request $request, string $type): Response
    {
        ...
        $form = $this->createForm(ImportType::class, null, [
            'action' => $this->generateUrl('import_sender_rule', ['type' => $type]),
            'user' => $this->getUser()
        ]);
    ...
    private function importSenderRule(string $pathFile, Domain $domain, string $rule): void
    {
        ...
        if (($handle = fopen($pathFile, "r"))) {
            while (($data = fgets($handle, 4096)) !== false) {
                ...
                $senderRule->setPriority(SenderRule::SENDER_RULE_PRIORITY_USER);
                $senderRule->setType(SenderRule::SENDER_RULE_TYPE_IMPORT);
