...
final class AmavisReleaseHandler
{
    ...
    public function __invoke(Message\AmavisRelease $amavisRelease): void
    {
        ...
        $lockName = "amavis-release-{$amavisRelease->getPartitionTag()}-{$amavisRelease->getMailId()}-{$amavisRelease->getRseqnum()}";
        $lock = $this->lockFactory->createLock($lockName, ttl: 10 * 60);

        if (!$lock->acquire()) {
            $logger->log->warning("Can't acquire the {$lockName} lock, the release is probably already running.");
            return;
        }
