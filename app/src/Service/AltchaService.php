...
use Microsoft\Graph\Generated\Models\ChallengingWord;
...
class AltchaService
{
    ...
    public function __construct(
        ...
        #[Autowire(env: 'ALTCHA_KEY_SIGNATURE')]
        private ?string $keySignature = null,
    ) {
        $this->altcha = new Altcha($this->keySecret ?: null, $this->keySignature ?: null);
        $this->pbkdf2 = new Pbkdf2();
    }

    public function buildChallenge(int $cost): Challenge
    {
    ...
    public function solveChallenge(Challenge $challenge): Solution
    {
        $solutionOptions = new SolveChallengeOptions(
            challenge: $challenge,
            algorithm: $this->pbkdf2,
        );

        $solution = $this->altcha->solveChallenge($solutionOptions);
    ...
    public function buildPayload(Challenge $challenge, Solution $solution): Payload
    {
