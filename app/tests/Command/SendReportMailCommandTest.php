...
class SendReportMailCommandTest extends KernelTestCase
{
    ...
    public function testReportMailIsSentToUserThatActivatedReport(): void
    {
        ...
        $mailer->expects($this->exactly(2))
            ->method('send')
            ->with(
                $this->callback(function (Email $email) use ($recipient, $recipient2, &$sentTo): bool {
        ...
        self::assertContains($recipient->getEmail(), $sentTo);
        self::assertContains($recipient2->getEmail(), $sentTo);
    }
