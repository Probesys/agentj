...
class PolicyControllerTest extends WebTestCase
{
    ...
    public function testUserCannotCreatePolicies(): void
    {
        $client = static::createClient();
        $user = UserFactory::new()->user()->create();
        $client->loginUser($user);
    ...
    public function testAdminCannotCreatePolicies(): void
    {
        $client = static::createClient();
        $admin = UserFactory::new()->admin()->create();
        $client->loginUser($admin);
    ...
    public function testSuperAdminCannotCreatePolicyWithInvalidCsrf(): void
    {
        $client = static::createClient();
        $superAdmin = UserFactory::new()->superAdmin()->create();
        $client->loginUser($superAdmin);
    ...
    public function testUserCannotEditPolicy(): void
    {
        $client = static::createClient();
        $user = UserFactory::new()->user()->create();
        $client->loginUser($user);
        ...
        self::assertSame($initialCount, PolicyFactory::count());
        $this->refresh($policy);
        self::assertSame($policyName, $policy->getPolicyName());
    }

    public function testAdminCannotEditPolicy(): void
    {
        $client = static::createClient();
        $admin = UserFactory::new()->admin()->create();
        $client->loginUser($admin);
    ...
    public function testSuperAdminCannotEditPolicyWithInvalidCsrfToken(): void
    {
        $client = static::createClient();
        $superAdmin = UserFactory::new()->superAdmin()->create();
        $client->loginUser($superAdmin);
        ...
        $newPolicyName = 'testNewPolicy';
        $payload = $this->getPayload($newPolicyName);
        $payload['policy']['_token'] = 'invalid CSRF token';
        $client->request('POST', '/policy/' . $policy->getId() . '/edit', $payload);
    ...
    public function testSuperAdminCanEditPolicy(): void
    {
        ...
        self::assertSame($initialCount, PolicyFactory::count());
        $this->refresh($policy);
        self::assertSame($newPolicyName, $policy->getPolicyName());
        self::assertResponseRedirects('/policy/?id=' . $policy->getId());
    }

    public function testUserCannotDeletePolicy(): void
    {
        $client = static::createClient();
        $user = UserFactory::new()->user()->create();
        $client->loginUser($user);
        ...
        self::assertSame(403, $client->getResponse()->getStatusCode());
        self::assertSame($initialCount, PolicyFactory::count());
        PolicyFactory::assert()->exists(['id' => $policy->getId()]);
    }

    public function testAdminCannotDeletePolicy(): void
    {
        $client = static::createClient();
        $admin = UserFactory::new()->admin()->create();
        $client->loginUser($admin);
        ...
        $payload = $this->getPayload($newPolicyName);
        $payload['policy']['_token'] = $this->generateCsrfToken($client, 'policy');
        $client->request('POST', '/policy/' . $policy->getId() . '/delete', $payload);
    ...
    public function testSuperAdminCannotDeletePolicyWithInvalidCsrfToken(): void
    {
        $client = static::createClient();
        $superAdmin = UserFactory::new()->superAdmin()->create();
        $client->loginUser($superAdmin);
