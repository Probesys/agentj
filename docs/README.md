# AgentJ documentation

## The Administrators' Guides

- [**Deploy in production**](/docs/administrators/deploy.md)
- [Infrastructure overview](/docs/infrastructure.md)
- [Login with OAuth2](/docs/administrators/oauth2.md)
- [Versioning policy](/docs/versioning.md)
- [Changelog of AgentJ](/CHANGELOG.md)
- [Administration via the web interface](/docs/administrators/webinterface.md)

## The Developers' Guides

- [**Install the development environment**](/docs/developers/install.md)
- [Infrastructure overview](/docs/infrastructure.md)
- [Git tips](/docs/developers/git.md)
- [Sending and receiving emails in development environment](/docs/developers/mail.md)
- [Running the linters](/docs/developers/linters.md)
- [Working with LDAP](/docs/developers/ldap.md)
- [Working with Xdebug](/docs/developers/xdebug.md)

Dedicated to the maintainers:

- [Release a version](/docs/developers/release.md)
- [Versioning policy](/docs/versioning.md)
