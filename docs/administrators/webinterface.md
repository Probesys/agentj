...
> [!IMPORTANT]
> Users connecting with this connector won't be imported automatically as your email server won't leak the entire list of the users. Even if you use this connector for authentication, you will have to create the users manually in AgentJ. See the dedicated section.
...
- Allow LDAP authentication: Whether or not you want to allow the users to login through the LDAP server.
- Send the daily report for created users: The value to set by default for new imported users concerning the daily report.
- Deactivate human authentication for created users: The value to set by default for new imported users concerning the humain authentication.
...
- Filtering rule
    - Antispam enabled: Let AgentJ filter incoming emails.
...
- Accept spam from authorized senders whose level is lower than: Allow to be more flexible with the previous rule for the authorized senders. It means mails sent by authorized senders can have a spam level up to this value until being considered as spam.
